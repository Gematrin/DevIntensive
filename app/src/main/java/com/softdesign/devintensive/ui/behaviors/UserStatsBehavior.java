...
public class UserStatsBehavior<V extends LinearLayout> extends AppBarLayout.ScrollingViewBehavior {
    private final int mMaxAppbarHeight;
    private final int mMinAppbarHeight;
    private final int mMaxUserStatsHeight;
    private final int mMinUserStatsHeight;

    public UserStatsBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.UserStatsBehavior);
        mMinUserStatsHeight = a.getDimensionPixelSize(R.styleable.UserStatsBehavior_behavior_min_height, 56);
        a.recycle();
        ...
        mMaxUserStatsHeight = context.getResources().getDimensionPixelSize(R.dimen.user_info_size);
    }
    ...
    public boolean onDependentViewChanged(CoordinatorLayout parent, View child, final View dependency) {
        ...
        int currentHeight = UiHelper.lerp(mMinUserStatsHeight, mMaxUserStatsHeight, currentFriction);
